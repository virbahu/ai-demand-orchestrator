import numpy as np
def forecast_demand_orchestrate(history, horizon=12, alpha=0.3, beta=0.15):
    n = len(history)
...
if __name__=="__main__":
    np.random.seed(42)
    hist = (np.cumsum(np.random.normal(5, 10, 52)) + 2500).tolist()
    r = forecast_demand_orchestrate(hist)
